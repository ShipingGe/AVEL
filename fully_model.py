...
import math
from transformer_decoder import TransformerDecoderLayer, MemoryAggregation
...
class MultipleBGClassifier(nn.Module):
    def __init__(self, hidden_dim, num_classes):
        super(MultipleBGClassifier, self).__init__()

        self.linear2 = nn.Linear(hidden_dim, num_classes)
        self.dropout = nn.Dropout(0.2)

    def forward(self, feats):
        preds = self.linear2(feats)
        preds = self.dropout(preds)
        preds = torch.sigmoid(preds)
        return preds
...
class MAML(nn.Module):
    def __init__(self, a_dim=128, v_dim=512, hidden_dim=256, category_num=29, num_layers=2, bank_size=32, margin=0.1):
    ...
    def fusion(self, audio, video):
        ...
        av_feats1 = self.spatial_decoder(a_spat, v_spat).reshape(B, -1,
                                                                 self.hidden_dim)  # audio feats that consider video feats
        ...
        av_feats2 = self.temporal_decoder(afeats.permute(1, 0, 2), vfeats.permute(1, 0, 2)).permute(1, 0,
                                                                                                    2)  # video feats that consider audio feats
    ...
    def forward(self, audio, video, one_hot_labels=None):
        ...
        # final_bg_predss = self.final_bg_classifier(fused_feats)
        final_evn_pred = self.final_event_classifier(fused_feats)

        if one_hot_labels is not None:

            labels_foreground = one_hot_labels[:, :, :-1]  # shape: [bs, 10, 28]
            ...
            relation_scores = (1 - jsd) + labels_bg
            ...
            rf_bg_loss = (labels_bg * max_dist).sum(1) / (labels_bg.sum(1) + 1e-12) + \
                         ((1 - labels_bg) * F.relu(self.margin - min_dist)).sum(1) / ((1 - labels_bg).sum(1) + 1e-12)
            ...
            return final_bg_preds, final_evn_pred, fused_feats
